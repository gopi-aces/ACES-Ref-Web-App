...
def BibTeX_abbr_New():
    ...
    if user_prompt:
        # Add system message explicitly before every user input if not already there
        if st.session_state.chat_history_abbr[0]["role"] != "system":
            st.session_state.chat_history_abbr.insert(0, SYSTEM_MESSAGE)
